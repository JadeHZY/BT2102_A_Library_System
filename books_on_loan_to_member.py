...
def books_on_loan_to_member():
    info = Toplevel()
    info.title("Books on Loan to Member Report")
    ...
    id_label = Label(info, text = "Membership ID:")
    id_entry = Entry(info, width=20)
    id_label.place(x=50, y=30)
    id_entry.place(x=38, y=70)
    
    def books_on_loan():
        memberid = id_entry.get()
        on_loan = Toplevel()
        on_loan.title("Books on Loan to Member Report")
        ...
        all_books_on_loan_command = "SELECT AccessionNumber FROM Borrow WHERE MemberID = " + "'" + memberid + "'"
        con = connection()
    ...
    search_bt = Button(info, text="Search Member", command = books_on_loan, bg='skyblue1')
    close_bt = Button(info, text="Back to Reports Menu", command = info.destroy, bg='skyblue1')
    search_bt.place(x=50, y=110)
    close_bt.place(x=30, y=155)